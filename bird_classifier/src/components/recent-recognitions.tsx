  ...
  return (
    <div className="bg-white/80 dark:bg-slate-800/80 rounded-xl shadow-md overflow-hidden">
      {recentBirds.length === 0 ? (
        <div className="p-6 text-center text-slate-500 dark:text-slate-400">No recent identifications</div>
      ) : (
        <ul className="divide-y divide-slate-200 dark:divide-slate-700">
          {recentBirds.map((bird) => (
            <li key={bird.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
              <Link href={`/bird/${bird.id}`} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-base font-medium text-slate-900 dark:text-slate-100 truncate">{bird.name}</h4>
                  <p className="text-sm text-slate-500 dark:text-slate-400 italic">{bird.scientificName}</p>
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-xs text-slate-500 dark:text-slate-400">{bird.time}</span>
