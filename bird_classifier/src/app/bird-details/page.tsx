...
export default function BirdDetailsPage() {
  ...
  const navigate = useNavigate(); // Added navigate for potential redirects

  useEffect(() => {
    console.log("Location state:", location.state); // Log state for debugging
    if (location.state?.bird && typeof location.state.bird === 'object' && location.state.bird.name) {
      setBird(location.state.bird);
      setError(null); // Clear any previous error
    } else {
      // If no valid bird data is passed, set an error
      setError("No bird details provided. Please select a bird from the gallery.");
    ...
    return (
      <div className="min-h-screen bg-gradient-green-blue flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-700 dark:text-slate-300">Loading bird details...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-4 py-8">
          <header className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              <Link to="/" className="p-2 mr-4 rounded-full bg-white/80 dark:bg-slate-800 shadow-sm hover:bg-white dark:hover:bg-slate-700 transition-colors">
                <ArrowLeft className="w-5 h-5 text-green-700 dark:text-green-400" />
                <span className="sr-only">Back to home</span>
              </Link>
              <h1 className="text-2xl md:text-3xl font-bold text-green-800 dark:text-green-400">Error</h1>
            </div>
            <ThemeToggle />
          </header>
          <main className="pb-16">
            <div className="bg-white/90 dark:bg-slate-800/90 rounded-xl shadow-lg overflow-hidden mb-8 p-6 text-center">
              ...
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">Could not load bird details</h2>
              <p className="text-slate-600 dark:text-slate-300 mb-6">{error || "An unknown error occurred."}</p>
              <Link to="/" className="inline-flex items-center gap-2 px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-full shadow-md transition-colors">
    ...
    <div className="min-h-screen bg-gradient-green-blue">
      <div className="container mx-auto px-4 py-8">
        ...
        <main className="pb-16">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-lg font-medium text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-2">
                    <Info className="w-5 h-5 text-green-600 dark:text-green-400" /> About
                  </h3>
                  <p className="text-slate-600 dark:text-slate-300">{bird.description}</p>
                </div>
                <div>
                  <h3 className="text-lg font-medium text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-2">
                    <MapPin className="w-5 h-5 text-green-600 dark:text-green-400" /> Habitat & Range
                  </h3>
          ...
          <div className="text-center">
            <Link to="/" className="inline-flex items-center gap-2 px-6 py-3 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-full shadow-md transition-colors">
              <span>Back to Gallery</span>
            </Link>
          </div>
